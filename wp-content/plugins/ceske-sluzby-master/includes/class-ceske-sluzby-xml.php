        ...
        if ( $varianta->is_in_stock() && $varianta->variation_is_visible() ) {
          ...
          if ( ! empty( $nazev_varianta ) ) {
            $xmlWriter->startElement( 'PRODUCTNAME' );
                ...
                $nazev_varianta_temp = preg_replace("[\r\n]", '', $nazev_varianta);

                preg_match('/(^\[\:sk\])(.*?)\[/', $nazev_varianta_temp, $nazev_varianta_matches);

                $nazev_varianta = $nazev_varianta_matches[2];

                if ( empty( trim($nazev_varianta) ) )  {
                    $nazev_varianta = $nazev_varianta_temp;
                }
          ...
          if ( ! empty( $popis_varianta ) ) {
            $xmlWriter->startElement( 'DESCRIPTION' );
                ...
                $popis_varianta_temp = preg_replace("[\r\n]", '', $popis_varianta);

                preg_match('/(^\[\:sk\])(.*?)\[/', $popis_varianta_temp, $popis_varianta_matches);

                $popis_varianta = $popis_varianta_matches[2];

                if ( empty( trim($popis_varianta) ) )  {
                    $popis_varianta = $popis_varianta_temp;
                }
        ...
        $xmlWriter->startElement( 'SHOPITEM' );
          ...
          if ( ! empty( $nazev_produkt ) ) {
            $xmlWriter->startElement( 'PRODUCTNAME' );
                ...
                $nazev_produkt_temp = preg_replace("[\r\n]", '', $nazev_produkt);

                preg_match('/(^\[\:sk\])(.*?)\[/', $nazev_produkt_temp, $nazev_produkt_matches);

                $nazev_produkt = $nazev_produkt_matches[2];

                if ( empty( trim($nazev_produkt) ) )  {
                    $nazev_produkt = $nazev_produkt_temp;
                }
          ...
          if ( ! empty( $popis_produkt ) ) {
            $xmlWriter->startElement( 'DESCRIPTION' );
                ...
                $popis_produkt_temp = preg_replace("[\r\n]", '', $popis_produkt);

                preg_match('/(^\[\:sk\])(.*?)\[/', $popis_produkt_temp, $popis_produkt_matches);

                $popis_produkt = $popis_produkt_matches[2];

                if ( empty( trim($popis_produkt) ) )  {
                    $popis_produkt = $popis_produkt_temp;
                }
