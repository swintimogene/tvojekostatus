...
if ( !class_exists( 'FME_Registration_Attributes_Front' ) ) { 
	...
	class FME_Registration_Attributes_Front extends FME_Registration_Attributes {
       ...
       function fme_save_extra_register_fields($customer_id) {
        	...
        	$message  = sprintf(__('Na e-shope www.tvojekostatus.sk sa registroval nový používateľ %s: '), get_option('blogname')) . "\n\n";
...
//				if($check!='') {
					...
					if($label->field_type=='checkbox' && $value==1) {
						$message .= sprintf(__($label->field_label.': %s'), 'Áno') . "\n\n";
					} else if($label->field_type=='checkbox' && ($value==0 || $value='' || is_null($value)) ) {
						$message .= sprintf(__($label->field_label.': %s'), 'Nie') . "\n\n";
...
//				}
	        ...
	        @wp_mail(get_option('admin_email'), sprintf(__('[%s] Registrácia nového používateľa'), get_option('blogname')), $message);
