...
function slide_anything_shortcode($atts) {
	...
	wp_register_script('owwl_carousel_js', SA_PLUGIN_PATH.'owwl-carousel/owwl.carousel.min.js', array('jquery'), '2.2.1', true);
	wp_enqueue_script('owwl_carousel_js');
	wp_register_style('owwl_carousel_css', SA_PLUGIN_PATH.'owwl-carousel/owwl.carousel.css', array(), '2.2.1.1', 'all');
	wp_enqueue_style('owwl_carousel_css');
	wp_register_style('owwl_theme_css', SA_PLUGIN_PATH.'owwl-carousel/sa-owwl-theme.css', array(), '2.0', 'all');
	wp_enqueue_style('owwl_theme_css');
	wp_register_style('owwl_animate_css', SA_PLUGIN_PATH.'owwl-carousel/animate.min.css', array(), '2.0', 'all');
	wp_enqueue_style('owwl_animate_css');
	if ($sa_pro_version) {
		...
		wp_register_script('owwl_thumbs_js', SA_PLUGIN_PATH.'owwl-carousel/owwl.carousel2.thumbs.min.js', array('jquery'), '0.1.8', true);
		wp_enqueue_script('owwl_thumbs_js');
	}
	...
	if ($id == 0) {
		...
		if (($post_status != 'publish') || (count($metadata) == 0)) {
			...
			// GENERATE HTML CODE FOR THE owwl CAROUSEL SLIDER
			...
			if ($slide_data['pagination'] == 'true') {
				if ($slide_data['autohide_arrows'] == 'true') {
					$additional_classes = "owwl-pagination-true autohide-arrows";
				} else {
					$additional_classes = "owwl-pagination-true";
				}
			} else {
				if ($slide_data['autohide_arrows'] == 'true') {
			...
			$output .= "<div id='".esc_attr($slide_data['css_id'])."' class='owwl-carousel sa_owwl_theme ".$additional_classes."' ";
			...
			for ($i = 1; $i <= $slide_data['num_slides']; $i++) {
				...
				if ($slide_data['lazy_load_images'] == '1') {
					// modify images (<img> tag) within slide content to enable owwl carousel lazy load
			...
			if (($sa_pro_version) && ($slide_data['thumbs_active'] == '1')) {
				...
				$output .= "<div id='".esc_attr($slide_data['css_id'])."_thumbs' class='sa_owwl_thumbs_wrap sa_thumbs_".$thumbs_loc.$add_classes."' style='".$thumbs_style."'>";
				$output .= "<div class='owwl-thumbs' data-slider-id='".esc_attr($slide_data['css_id'])."'>";
				for ($i = 1; $i <= $slide_data['num_slides']; $i++) {
					...
					$output .= "<div class='owwl-thumb-item' style='".$thumb_style."' title='Slide ".$i."'></div>";
				}
				$output .= "</div>";		// .sa_owwl_thumbs
				$output .= "</div>\n";	// .sa_owwl_thumbs_wrap
			}
			...
			if ($slide_data['showcase_slider'] == '1') {
				$output .= "<div id='showcase_nav' class='owwl-nav'></div>\n";
			}
			...
			// ### GENERATE JQUERY CODE FOR THE owwl CAROUSEL SLIDER ###
			...
			$output .= "		jQuery('#".esc_attr($slide_data['css_id'])."').owwlCarousel({\n";
			if ($single_item == 1) {
				$output .= "			items : 1,\n";
			...
			if ($slide_data['hero_slider'] != '1') {
				...
				if (($slide_min_height != '') && ($slide_min_height != '0')) {
					...
					$output .= "			jQuery('#".esc_attr($slide_data['css_id'])." .owwl-item .sa_hover_container').css('min-height', slide_height+'px');\n";
					$output .= "		}\n";
				}
			}
			...
			if ($slide_data['showcase_slider'] == '1') {
				...
				$output .= "		jQuery('#".esc_attr($slide_data['css_id'])."').on('translated.owwl.carousel resized.owwl.carousel', function(event) {\n";
				$output .= "			set_first_last_active_classes('".esc_attr($slide_data['css_id'])."');\n";
				$output .= "		});\n";
				$output .= "		function set_first_last_active_classes(css_id) {\n";
				$output .= "			var total = jQuery('#".esc_attr($slide_data['css_id'])." .owwl-stage .owwl-item.active').length;\n";
				$output .= "			jQuery('#".esc_attr($slide_data['css_id'])." .owwl-stage .owwl-item').removeClass('sc_partial');\n";
				$output .= "			jQuery('#".esc_attr($slide_data['css_id'])." .owwl-stage .owwl-item.active').each(function(index){\n";
			...
			if (($sa_pro_version) && ($slide_data['thumbs_active'] == '1')) {
				...
				if ($slide_data['thumbs_border_width'] > 0) {
					...
					$output .= "		jQuery('#".esc_attr($slide_data['css_id'])."_thumbs .owwl-thumbs .active').css('border-color', '".$slide_data['thumbs_border_color']."');\n";
					$output .= "		var owwl = jQuery('#".esc_attr($slide_data['css_id'])."');\n";
					// 					owwl carousel change event - set border colour of the active thumb
					$output .= "		owwl.on('changed.owwl.carousel', function(event) {\n";
					$output .= "			jQuery('#".esc_attr($slide_data['css_id'])."_thumbs .owwl-thumbs .owwl-thumb-item').css('border-color', 'transparent');\n";
					$output .= "			jQuery('#".esc_attr($slide_data['css_id'])."_thumbs .owwl-thumbs .active').css('border-color', '".$slide_data['thumbs_border_color']."');\n";
					$output .= "		})\n";
				}
				...
				$output .= "				jQuery('#".esc_attr($slide_data['css_id'])."_thumbs .owwl-thumbs .owwl-thumb-item').css('width', mobile_width);\n";
				$output .= "				jQuery('#".esc_attr($slide_data['css_id'])."_thumbs .owwl-thumbs .owwl-thumb-item').css('height', mobile_height);\n";
				$output .= "			} else if ((tablet_perc != 0) && (win_width < 1000)) {\n";
				$output .= "				jQuery('#".esc_attr($slide_data['css_id'])."_thumbs .owwl-thumbs .owwl-thumb-item').css('width', tablet_width);\n";
				$output .= "				jQuery('#".esc_attr($slide_data['css_id'])."_thumbs .owwl-thumbs .owwl-thumb-item').css('height', tablet_height);\n";
				$output .= "			} else {\n";
				$output .= "				jQuery('#".esc_attr($slide_data['css_id'])."_thumbs .owwl-thumbs .owwl-thumb-item').css('width', '".$slide_data['thumbs_width']."px');\n";
				$output .= "				jQuery('#".esc_attr($slide_data['css_id'])."_thumbs .owwl-thumbs .owwl-thumb-item').css('height', '".$slide_data['thumbs_height']."px');\n";
				$output .= "			}\n";
			...
			if (($sa_pro_version) && ($lightbox_count > 0)) {
				...
				if ($slide_data['slide_duration'] != 0) {
					...
					$output .= "				jQuery('#".esc_attr($slide_data['css_id'])."').trigger('stop.owwl.autoplay');\n";
					$output .= "			},\n";
					$output .= "			close: function() {\n";
					$output .= "				jQuery('#".esc_attr($slide_data['css_id'])."').trigger('play.owwl.autoplay');\n";
					$output .= "			}\n";
					$output .= "		},\n";
				}
				$output .= "		type: 'image'\n";
...
// ### MODIFY IMAGES (<img> tag) WITHIN STRING PASSED (SLIDE CONTENT) TO ENABLE owwl CAROUSEL LAZY LOAD ###
function set_slide_images_to_lazy_load($slide_content) {
	...
	// 2) FOR EACH <IMG> TAG WITHIN THE SLIDE CONTENT, ADD THE 'owwl-lazy' CLASS
	$dom = new DOMDocument();
	...
	foreach ($imgs as $img) {
		...
		if ($curr_class != '') {
			$img->setAttribute('class', $curr_class.' owwl-lazy');
		} else {
			$img->setAttribute('class', 'owwl-lazy');
		}
	}
	$slide_content = $dom->saveHTML();
