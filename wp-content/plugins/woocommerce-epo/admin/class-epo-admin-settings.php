...
if(!class_exists('EPO_Admin_Settings')):

abstract class EPO_Admin_Settings {
	protected $page_id = '';	
	...
	public function __construct($page, $section = '') {
        ...
        $this->tabs = array( 'general_settings' => 'Product Options', 'advanced_settings' => 'Advanced Settings' );
	}
	...
	public function render_form_field_element($field, $atts = array(), $render_cell = true){
		if($field && is_array($field)){
			...
			$flabel    = isset($field['label']) && !empty($field['label']) ? EPO_i18n::__t($field['label']) : '';
			$sub_label = isset($field['sub_label']) && !empty($field['sub_label']) ? EPO_i18n::__t($field['sub_label']) : '';
			$tooltip   = isset($field['hint_text']) && !empty($field['hint_text']) ? EPO_i18n::__t($field['hint_text']) : '';
	...
	private function render_form_field_element_select($field, $atts = array()){
		...
		if($field && is_array($field)){
			...
			foreach($field['options'] as $value => $label){
				...
				$field_html .= '<option value="'. trim($value) .'" '.$selected.'>'. EPO_i18n::__t($label) .'</option>';
			}
			$field_html .= '</select>';
		}
		return $field_html;
	...
	private function render_form_field_element_multiselect($field, $atts = array()){
		...
		if($field && is_array($field)){
			...
			$field_html = '<select multiple="multiple" '. $field_props .' class="epo-enhanced-multi-select" >';
			foreach($field['options'] as $value => $label){
				//$selected = $value === $fvalue ? 'selected' : '';
				$field_html .= '<option value="'. trim($value) .'" >'. EPO_i18n::__t($label) .'</option>';
			}
			$field_html .= '</select>';
		}
		return $field_html;
	...
	public function render_form_fragment_tooltip($tooltip = false){
		if($tooltip){
			...
			<td style="width: 26px; padding:0px;">
				<a href="javascript:void(0)" title="<?php echo $tooltip; ?>" class="epo_tooltip"><img src="<?php echo EPO_ASSETS_URL_ADMIN; ?>/css/help.png" title=""/></a>
			</td>
			<?php
		}else{
			?>
