...
class EPO_Activator {
	...
	public static function check_for_premium_settings(){
		$premium_settings = get_option(EPO_Utils::OPTION_KEY_CUSTOM_SECTIONS);
	...
	public static function may_copy_free_version_settings(){
		$extra_options = get_option('epof_custom_product_fields');
			...
			if($fields){
				foreach($fields as $field_name => $field){
					try{
						...
						$section = EPO_Utils_Section::add_field($section, $new_field);
					} catch (Exception $e) {
					}
				}
			}
	...
	public static function prepare_fields($field){
		...
		if($field){
			...
			$new_field = EPO_Utils_Field::create_field($type);
			...
			$new_field = EPO_Utils_Field::prepare_properties($new_field);
		}
		return $new_field;
	}
}
