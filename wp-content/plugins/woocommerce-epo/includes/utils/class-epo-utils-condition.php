...
if(!class_exists('EPO_Utils_Condition')):

class EPO_Utils_Condition {
	const LOGIC_AND = 'and';
	const LOGIC_OR  = 'or';
	...
	public static function is_satisfied($rules_set_list, $product, $categories){
		$valid = true;
		if(is_array($rules_set_list) && !empty($rules_set_list)){
			foreach($rules_set_list as $rules_set){				
				if(!self::is_satisfied_rules_set($rules_set, $product, $categories)){
					$valid = false;
				}
			}
		}
	...
	private static function is_satisfied_condition($condition, $product, $categories){
		$satisfied = true;
		if(self::is_valid_condition($condition)){
			...
			if(!empty($operands) && !in_array(-1, $operands)){
				if($op_type == self::OP_TYPE_PRODUCT){
					...
					$user_roles = EPO_Utils::get_user_roles();
	...
	private static function is_satisfied_rules_set($rules_set, $product, $categories){
		$satisfied = true;
		...
		if(!empty($condition_rules)){
			if($logic === self::LOGIC_AND){			
				foreach($condition_rules as $condition_rule){				
					if(!self::is_satisfied_rule($condition_rule, $product, $categories)){
						$satisfied = false;
						break;
					}
				}
	...
	private static function is_satisfied_rule($rule, $product, $categories){
		$satisfied = true;
		...
		if(!empty($conditions_set_list)){
			if($logic === self::LOGIC_AND){			
				foreach($conditions_set_list as $conditions_set){				
					if(!self::is_satisfied_conditions_set($conditions_set, $product, $categories)){
						$satisfied = false;
						break;
					}
				}
	...
	private static function is_satisfied_conditions_set($conditions_set, $product, $categories){
		$satisfied = true;
		...
		if(!empty($conditions)){			 
			if($logic === self::LOGIC_AND){			
				foreach($conditions as $condition){				
					if(!self::is_satisfied_condition($condition, $product, $categories)){
						$satisfied = false;
						break;
					}
				}
