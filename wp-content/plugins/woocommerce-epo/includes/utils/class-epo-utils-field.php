...
if(!class_exists('EPO_Utils_Field')):

class EPO_Utils_Field {
	...
	public static function is_show_field($field, $product, $categories){
		$rules_set_list = $field->get_property('conditional_rules');
		$valid = EPO_Utils_Condition::is_satisfied($rules_set_list, $product, $categories);
		...
		$show = apply_filters('epo_show_field', $show, $field->name);
		return $show;
	}
	...
	public static function prepare_field_from_posted_data($posted, $props){
		...
		if($type === 'select' || $type === 'multiselect' || $type === 'radio' || $type === 'checkboxgroup'){
			...
			$options_extra = apply_filters('epo_field_options', array(), $field->get_property('name'));
		...
		$field->set_property('conditional_rules', EPO_Utils_Condition::prepare_conditional_rules($posted, false));
		...
		$field->set_property('conditional_rules_ajax', EPO_Utils_Condition::prepare_conditional_rules($posted, true));
	...
	public static function add_wpml_support($field){
		EPO_i18n::wpml_register_string('Field Title - '.$field->name, $field->title );
		EPO_i18n::wpml_register_string('Field Subtitle - '.$field->name, $field->subtitle );
		EPO_i18n::wpml_register_string('Field Placeholder - '.$field->name, $field->placeholder );
		
		$options = $field->get_property('options');
		foreach($options as $option){
			EPO_i18n::wpml_register_string('Field Option - '.$field->name.' - '.$option['key'], $option['text'] );
		}
	}
	...
	private static function get_title_html($field, $skip_id = true){
		...
		if($field->get_property('title')){
			$title_class = EPO_Utils::convert_cssclass_string($field->get_property('title_class'));
			...
			$title_html .= stripslashes(EPO_i18n::__t($field->get_property('title')));
			$title_html .= '</'.$title_type.'>';
		}
		
		$subtitle_html = '';
		if($field->get_property('subtitle')){
			$subtitle_class = EPO_Utils::convert_cssclass_string($field->get_property('subtitle_class'));
			...
			$subtitle_html .= EPO_i18n::__t($field->get_property('subtitle'));
	...
	private static function prepare_field_value($field){
		...
		$value = apply_filters('epo_product_extra_option_value_'.$name, $field->get_property('value'));
	...
	private static function prepare_field_props($field, $is_multiple = false, $skip_value = false, $skip_placeholder = false){
		...
		$placeholder = $field->get_property('placeholder') ? EPO_i18n::esc_html__t($field->get_property('placeholder')) : false;
		$is_price_field = self::is_price_field($field);
		$name_suffix = $is_multiple ? '[]' : '';
		
		$price_data  = self::get_price_data($field);
		$input_class = $is_price_field ? 'epo-price-field' : '';
		...
		if($i_class){
			$input_class .= ' '.EPO_Utils::convert_cssclass_string($i_class);
		}
		
		if($is_price_field && ($type === 'select' || $type === 'multiselect')){
			$input_class .= ' epo-price-option-field';
		}
		
		$disable_select2 = EPO_Utils::get_settings('disable_select2_for_select_fields') ? true : false; 
		
		if($type === 'multiselect' && !$disable_select2){
			$input_class .= ' epo-enhanced-multi-select';
			
		}else if($type === 'select' && !$disable_select2){
			$input_class .= ' epo-enhanced-select';
			...
			$input_class .= ' epo-date-picker';
			
		}else if($type === 'timepicker'){
			$input_class .= ' epo-time-picker input-text';
		}
		
		$required_html = '';
		if($field->get_property('required')){
			$required_html .= apply_filters('epo_enable_html5_required_validation', true, $name) ? ' required' : '';
			$input_class .= ' validate-required';
		}
		
		$field_props  = 'id="'.$name.'" name="'.$name.$name_suffix.'"';
		$field_props .= !$skip_value ? ' value="'.$value.'"' : '';
		$field_props .= $placeholder && !$skip_placeholder ? ' placeholder="'.$placeholder.'"' : '';
		$field_props .= ' class="epo-input-field '.trim($input_class).'"';
	...
	private static function prepare_field_options($field){
		...
		$value = self::prepare_field_value($field);
		$is_price_field = self::is_price_field($field);
		$show_price = apply_filters('epo_display_field_option_price', true, $name, $field->get_property('type'));
		$skip_i18n = apply_filters('epo_skip_translation_for_numeric_field_options', true, $name);
		$field_options = apply_filters('epo_input_field_options', $field->get_property('options'), $name);
		...
		foreach($field_options as $option_key => $option){		
			...
			if(!$skip_translation){
				$option_text = EPO_i18n::esc_html__t($option_text);
			}
	...
	private static function get_required_html($field){
		$html = '';
		if($field->get_property('required')){
			$title_required = __('required', 'woocommerce-epo');
			$html = apply_filters( 'epo_required_html', ' <abbr class="required" title="'.$title_required.'">*</abbr>', $field->get_property('name') );
		}
		return $html;
	}
	...
	private static function get_price_html_option($is_price_field, $option, $name){
		...
		if($is_price_field){
			...
			if(is_numeric($price) && $price != 0){
				...
				if($price_html){
					$price_prefix = apply_filters('epo_extra_cost_display_prefix', ' (', $name, $price, $price_type);
					$price_suffix = apply_filters('epo_extra_cost_display_suffix', ')', $name, $price, $price_type);
	...
	private static function get_price_html($is_price_field, $price_type, $price, $field = false){
		...
		if($price_type != 'custom' && $is_price_field){
			if($price_type === 'percentage'){
				$html = $price.apply_filters('epo_extra_price_percentage_symbol', '%', $field);
			}else if($price_type === 'dynamic' || $price_type === 'dynamic-excl-base-price'){
				if($field){
					$name = $field->get_property('name');
					$price_html = EPO_Utils_Price::display_price($price, $field);
					$price_unit = $field->get_property('price_unit');
					$price_unit_label = apply_filters('epo_extra_cost_unit_label_'.$name, '/'.$price_unit.' unit', $price_unit);
					$html = $price_html.$price_unit_label;
				}
			}else{
				$html = EPO_Utils_Price::display_price($price, $field);
			}
		}
		return $html;
	}
	...
	private static function get_display_price($field){
		...
		if($is_price_field){
			...
			if($product){
				...
				$price = apply_filters('epo_extra_cost_unit_price_'.$name, $price, $product_price, $price_type);
			}
	...
	private static function get_char_counter_html($field){
		...
		$html = '';
		if($class_arr && (in_array("epo-char-count", $class_arr) || in_array("epo-char-left", $class_arr)) ) {
	...
	private static function prepare_field_html_input($field, $section, $input_html, $show_price = true){
		...
		if($input_html){
			$field_type = $field->get_property('type');
			$show_price = apply_filters('epo_display_field_price', $show_price, $field->get_property('name'), $field->get_property('type'));
			...
			$wrapper_class  = EPO_Utils::convert_cssclass_string($field->get_property('cssclass'));
			
			$conditions_data = self::get_ajax_conditions_data($field);
			if($conditions_data){
				$wrapper_class .= empty($wrapper_class) ? 'epo-conditional-field' : ' epo-conditional-field';
			}
	...
	private static function get_html_radio($field, $section){
		...
		$cssclass = EPO_Utils::convert_cssclass_array($field->get_property('cssclass'));
		$input_class = $is_price_field ? 'epo-price-field' : '';
		$title_class = EPO_Utils::convert_cssclass_string($field->get_property('title_class'));
		...
		foreach($field->get_property('options') as $option_key => $option){
			...
			$price_data  = self::get_price_data_option($field, $option);
			$option_text = EPO_i18n::esc_html__t($option['text']);
			...
			$field_props  = 'value="'.$option_key.'" class="epo-input-field '.trim($input_class).'" '.$checked;
			$field_props .= $price_data;
			$field_props .= $required_html;
	...
	private static function get_html_checkbox($field, $section){
		...
		$value = apply_filters('epo_product_extra_option_value_'.$name, $value);
		$title_class = EPO_Utils::convert_cssclass_string($field->get_property('title_class'));
		...
		if($field->get_property('subtitle') && apply_filters('epo_checkbox_field_label_wrap', true, $name)){
		...
		//$input_html .= EPO_i18n::__t($field->get_property('title'));
	...
	private static function get_html_checkboxgroup($field, $section){
		...
		$title_class = EPO_Utils::convert_cssclass_string($field->get_property('title_class'));
		$show_price = apply_filters('epo_display_field_option_price', true, $name, $field->get_property('type'));
		$options_per_line = apply_filters('epo_checkboxgroup_options_per_line', 1, $name);
		...
		foreach($field->get_property('options') as $option_key => $option){
			...
			$field_props  = 'id="'.$option_id.'" name="'. esc_attr($name) .'[]"';
			$field_props .= ' value="'.$option_key.'" class="epo-input-field input-checkbox '.trim($input_class).'" '.$checked;
			$field_props .= $price_data;
	...
	private static function get_html_datepicker($field, $section){
		...
		$firstDay = apply_filters('epo_date_picker_first_day', 0, $name);
		$display_inline = apply_filters('epo_date_picker_display_inline', false, $name);
		
		$dp_props  = ' data-date-format="'. $field->get_property('date_format') .'" data-default-date="'. $field->get_property('default_date') .'"';
		$dp_props .= ' data-min-date="'. $field->get_property('min_date') .'" data-max-date="'. $field->get_property('max_date') .'"';
		$dp_props .= ' data-year-range="'. $field->get_property('year_range') .'" data-number-months="'. $field->get_property('number_of_months') .'"';
		$dp_props .= ' data-disabled-days="'. $field->get_property('disabled_days') .'" data-disabled-dates="'. $field->get_property('disabled_dates') .'"';
		$dp_props .= ' data-first-day="'. $firstDay .'"';
		
		if($display_inline){
			$field_props = str_replace('epo-date-picker', '', $field_props);
			$dp_props .= ' data-alt-field="'. $name .'"';

			$input_html  = '<input type="hidden" '.$field_props.' />';
			$input_html .= '<span '.$dp_props.' class="epo-date-picker"></span>';
		}else{
	...
	private static function get_html_label($field, $section){
		...
		if($field->get_property('title')){
			...
			$title_html .= EPO_i18n::__t($field->get_property('title'));
			$title_html .= '</'.$title_type.'>';
		}
