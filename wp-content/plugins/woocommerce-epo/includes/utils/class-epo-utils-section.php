...
if(!class_exists('EPO_Utils_Section')):

class EPO_Utils_Section {
	...
	public static function is_show_section($section, $product, $categories){
		/*$show = true;
		...
		$show = apply_filters('epo_show_section', $show, $section->name);
		return $show;
	}
	...
	public static function prepare_section_from_posted_data($posted, $form = 'new'){
		...
		if($form === 'edit'){
			$section = EPO_Admin_Utils::get_section($name);
		}else{
		...
		$section->set_property('conditional_rules', EPO_Utils_Condition::prepare_conditional_rules($posted, false));
		...
		$section->set_property('conditional_rules_ajax', EPO_Utils_Condition::prepare_conditional_rules($posted, true));
	...
	public static function add_field($section, $field){
		if(self::is_valid_section($section) && EPO_Utils_Field::is_valid_field($field)){
	...
	public static function get_product_sections_and_fields($product){
		...
		if($sections && is_array($sections) && !empty($sections)){
			foreach($sections as $section_name => $section){
				$section = self::prepare_section_and_fields($section, $product_id, $categories);
				if($section){
	...
	public static function prepare_section_and_fields($section, $product_id, $categories){
		if(self::is_valid_section($section) && self::is_show_section($section, $product_id, $categories)){					
			$fields = self::get_fields($section);
			if($fields && is_array($fields)){
				foreach($fields as $field_name => $field){
					if(EPO_Utils_Field::is_enabled($field) && EPO_Utils_Field::is_show_field($field, $product_id, $categories)){
	...
	public static function get_product_fields($product, $names_only = true){
		...
		if($sections && is_array($sections) && !empty($sections)){
			foreach($sections as $section_name => $section){
				$fields = self::get_product_section_fields($section, $product_id, $categories, $names_only);
				if($fields){
	...
	public static function get_product_section_fields($section, $product_id, $categories, $names_only = true){
		$prod_fields = array();
		if(self::is_valid_section($section) && self::is_show_section($section, $product_id, $categories)){					
			$fields = self::get_fields($section);
			if($fields && is_array($fields)){
	...
	public static function add_wpml_support($section){
		EPO_i18n::wpml_register_string('Section Title - '.$section->name, $section->title );
		EPO_i18n::wpml_register_string('Section Subtitle - '.$section->name, $section->subtitle );
	}
		...
		if(is_array($fields)){
			foreach($fields as $field){
				if($field->get_property('type') === 'hidden'){
					$field_html_hidden .= EPO_Utils_Field::prepare_field_html($field, $section);
				}else{
					$field_html .= EPO_Utils_Field::prepare_field_html($field, $section);
				}
			}
		}
		
		if(!empty($field_html)){
			$cssclass  = EPO_Utils::convert_cssclass_string($section->get_property('cssclass'));
			$cssclass .= $product_type ? ' epo_'.$product_type : '';
			
			$conditions_data = self::get_ajax_conditions_data($section);
			if($conditions_data){
				$cssclass .= empty($cssclass) ? 'epo-conditional-section' : ' epo-conditional-section';
			}
	...
	public static function prepare_title_html($section){
		...
		if($section->get_property('title')){
			$title_class = EPO_Utils::convert_cssclass_string($section->get_property('title_class'));
			...
			$title_html .= EPO_i18n::esc_html__t($section->get_property('title'));
			$title_html .= '</'.$title_type.'>';
		}
		
		$subtitle_html = '';
		if($section->get_property('subtitle')){
			$subtitle_class = EPO_Utils::convert_cssclass_string($section->get_property('subtitle_class'));
			...
			$subtitle_html .= EPO_i18n::esc_html__t($section->get_property('subtitle'));
